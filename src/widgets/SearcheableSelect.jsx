...
const SearcheableSelect = ({
    name,label,required,Sync,isNotProvider,h,
    options,tooltipText,defaultValue,onChange,...rest}) => {
    ...
    return (
    <Box>
        <Text w='full' display={'inline-block'} my='1rem' color={'rgba(4, 9, 33, 0.76)'} 
         fontSize={'13px'}  as='label' htmlFor={name}>
            {label} 
            { required? <span>*</span>:''}
            {renderJSX(
                Sync && isNotProvider,
        ...
        <Box as='details' ref={menuRef} position={'relative'} h={h || '50px'}>
            <Box h={h || '50px'}  as='summary'>
                ...
                <Input bg='#E8E8E8'  h={h || '50px'} w='full' 
                 position={'absolute'} onBlur={closeOptions}
                 top={0} left={0} onFocus={expandOptions}
                 {...rest}
                /> 
            </Box>
            ...
            <Box 
             ...
             p={'0.6em 0.4em'} bg-='white' 
             boxShadow={'0 3px 10px rgb(0 0 0 / 0.2)'}
             zIndex={100}
             position={'absolute'} top='115%' left={0} w='full'>
                {optionArr.map((value,key)=>{
                        const mt = key?'0.8em':0
                        return(
                            <Text onClick={()=>{
                                console.log('clicking open')
                            }} fontSize={'12px'} mt={mt} key={key}> 
                                {value} 
                            </Text>
                        )
                    }
