...
const SelectInput = ({name,label,required,Sync,isNotProvider,tooltipText,options,defaultValue,onChange,...rest})=>{
    ...
    return( 
        <Box>
            ...
            <Text display={'inline-block'} my='1rem' color={'rgba(4, 9, 33, 0.76)'} fontSize={'13px'}  as='label' htmlFor={name}>
                {label} 
                { required? <span>*</span>:''}
                {renderJSX(
                  Sync && isNotProvider,
